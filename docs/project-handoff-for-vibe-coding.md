...
| RAG 体系完善·阶段 3 | D 知识库运营闭环：多 collection（`QdrantVectorStore.from_settings(..., collection_name=)` + `GET /api/knowledge-base/collections` 列出 kb_* 库与旧库状态）+ 文档 CRUD（`POST/PUT/DELETE /api/knowledge-base/documents` + `POST .../ingest`，Python 落盘 `data/knowledge_base/` + Qdrant 同步 + 经 `KnowledgeDocumentClient` 调 Java 内部接口持久化 `knowledge_documents` 元数据）+ Java `/internal/knowledge-documents` 写接口（POST upsert / DELETE，五头鉴权）+ 前端 KnowledgeBaseView 上传/编辑/删除/单独同步 + collection 状态 + 趋势图 RAG 检索命中率第三条线（history 已含 rag_retrieval） | 已完成（真实验收：上传→对话引用新文档 score 0.929→编辑改赔付天数→对话更新→删除→Qdrant 清空+对话不再引用；Python 1568 passed / Java 87 / 前端 build ✓） |
