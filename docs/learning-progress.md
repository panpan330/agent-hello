...
| 6 | LLM 调用日志安全 | 已完成 | `notes/stage10-06-llm-call-log-safety.md`、`app/core/llm_logging_safety.py`、`tests/test_llm_logging_safety.py`、`llm_service.py`；系统学习 LLM 调用日志安全边界，明确成功/失败/流式调用只记录 provider、model、operation、outcome、elapsed_ms、token、error_code、status_code、trace_id 等安全元信息，禁止 prompt、messages、用户输入、完整模型回答、API Key、Authorization、工具结果和 RAG 文档正文进入日志，并用安全 payload、受保护字段、敏感字段过滤和测试固定规则 |
| 7 | 配置与密钥管理 | 已完成 | `notes/stage10-07-config-and-secret-management.md`、`app/core/config_safety.py`、`tests/test_config_safety.py`、`.env.example`、`health.py`；系统学习 `.env`、`.env.example`、环境变量、配置与密钥区别、不同环境配置、API Key 管理、密钥不上 GitHub、配置分层、最小暴露、安全配置快照、密钥配置检查和 `/ready` 对真实 LLM 模式缺 key 的清晰失败 |
