...
| M10 | 当前阶段 | AI 应用生产化与可运营能力 | 28 节主线，补 tracing、日志安全、配置与密钥、token 成本、耗时拆解、多模型路由、fallback、成本控制、限流、重试、超时、SSE 生产化、安全加固、自动化评估、监控告警、灰度回滚和 Runbook |
...
| 16 | SSE 流式输出生产化 | 优化流式响应格式、体验和资源占用 |
| 17 | 流式错误处理和中断 | 处理中途失败、用户取消和部分输出 |
| 18 | Prompt Injection 防护强化 | 加固系统提示、工具调用、外部输入和多轮上下文 |
| 19 | 权限控制强化 | 梳理用户、租户、角色、工具权限和最终授权 |
| 20 | 模型输出安全与隐私保护 | 处理敏感信息、隐私、日志脱敏和安全回答边界 |
| 21 | 自动化评估平台基础 | 从脚本式 eval 走向持续评估 |
| 22 | 评测集版本管理 | 管理评测集变更和回归基线 |
| 23 | Bad Case 收集和回归测试 | 把线上问题沉淀为回归测试 |
| 24 | 线上监控指标设计 | 设计请求量、错误率、延迟、token、成本、fallback 等指标 |
| 25 | 告警 | 设计有效告警，避免漏报和噪音 |
| 26 | 灰度发布与回滚 | 学习 Prompt、模型、RAG 参数和路由策略的灰度与回滚 |
| 27 | SLO / SLA / Runbook | 理解服务目标、对外承诺和故障处理手册 |
| 28 | 阶段 10 总复盘和面试表达强化 | 整理生产化能力的项目表达和面试回答 |
